...
workflows/
  ...
  03-ai-publishing-gate      AI classification + the 5 publish rules  (M9, next)
...
`02 — Source Ingestion` currently ends at the normaliser. M9 attaches the AI
classification and the Publishing Gate to that output; M10 adds persistent retry
state and the health snapshot; M11 adds the newsletter. The scheduler already
respects `next_retry_at` so M10 slots in without changing it.
